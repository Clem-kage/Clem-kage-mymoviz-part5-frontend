...
function Movie(props) {
  ...
  // const descTrick = props.overview.slice(0, 50);
  // const formatDesc =  <p>{descTrick}</p>
  ...
  const renderOverview = (arg) => arg.length > 50? arg.slice(0, 150): arg;
  
 
  console.log(props.overview)
  return (
    <div className={styles.card}>
      <img className={styles.image} src={`https://www.themoviedb.org/t/p/original/${props.poster}`} alt={props.title} />
      <div className={styles.textContainer}>
        <div>
          <span className={styles.name}>{props.title}</span>
          <p className={styles.description}>{props.overview && renderOverview(props.overview)}</p>
        </div>
